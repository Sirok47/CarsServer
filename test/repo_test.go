...
func TestRepository(t *testing.T) {
	...
	err := r.SignUp(context.Background(), model.UserParams{Nick: "keklik_tester", Password: "glazirok"})
	if err != nil {
		t.Errorf("SignUp() got err %v", err.Error())
	}
	dbconn.Exec(context.Background(), "delete from users where nick = keklik_tester")
	_, err = r.LogIn(context.Background(), model.UserParams{Nick: "keklik", Password: "qpwoeirutyM123"})
	if err != nil {
		t.Errorf("LogIn() got err %v", err.Error())
	}
	err = r.Create(context.Background(), model.CarParams{CarBrand: "test", CarNumber: 1331, CarType: "test", Mileage: 1000})
	if err != nil {
		t.Errorf("Create() got err %v", err.Error())
	}
	err = r.Update(context.Background(), &model.CarParams{CarNumber: 1331, Mileage: 1111})
	if err != nil {
